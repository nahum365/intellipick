...
export function updateScorePanel(panel) {
  ...
  const winPct =
    decided > 0 ? Math.round((perf.correct / decided) * 100) : null;
  ...
  let statusLine = "";
  if (perf.live > 0)
    statusLine += `<span style="color:var(--upset)">${perf.live} live</span>`;
  if (perf.pending > 0)
    statusLine += `${statusLine ? " &middot; " : ""}${perf.pending} upcoming`;
  ...
  function buildUpsetRow(u, pickId, matchup) {
    ...
    row.style.cssText =
      "font-size:11px;padding:4px 0;border-bottom:1px solid var(--border-light);display:flex;align-items:center;gap:6px";
    ...
    if (teamProfile) {
      ...
      teamSpan.addEventListener("mouseenter", () =>
        showTooltip(u.team, teamProfile, matchup, teamSpan),
      );
    ...
    if (oppProfile) {
      ...
      oppSpan.addEventListener("mouseenter", () =>
        showTooltip(u.opponent, oppProfile, matchup, oppSpan),
      );
    ...
    confSpan.style.cssText =
      "font-size:9px;font-weight:700;color:var(--upset);flex-shrink:0";
    ...
    if (liveScore && liveScore.status === "final" && matchup) {
      ...
      const upsetTeamScore = upsetTeamIsTeam1
        ? liveScore.team1Score
        : liveScore.team2Score;
      const otherScore = upsetTeamIsTeam1
        ? liveScore.team2Score
        : liveScore.team1Score;
    ...
    if (matchup) {
      ...
      infoBtn.style.cssText =
        "background:none;border:1px solid var(--border);border-radius:4px;cursor:pointer;font-size:10px;color:var(--text-muted);padding:1px 5px;flex-shrink:0;line-height:1.2";
  ...
  if (r64Upsets.length > 0) {
    ...
    r64Title.style.cssText =
      "font-size:11px;font-weight:800;text-transform:uppercase;letter-spacing:0.8px;color:var(--text-muted);margin-bottom:6px";
