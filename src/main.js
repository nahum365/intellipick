...
import {
  createInsightsBar,
  updateInsightsBar,
} from "./components/insightsBar.js";
...
function getActiveRound() {
  ...
  for (const round of roundOrder) {
    ...
    if (
      matchups.some((m) => {
        ...
        return (
          score && (score.status === "live" || score.status === "halftime")
        );
      })
    )
      return round;
  ...
  for (let i = roundOrder.length - 1; i >= 0; i--) {
    if (
      getMobileMatchupsForRound(roundOrder[i]).some((g) => g.items.length > 0)
    )
      return roundOrder[i];
  }
...
function getMobileMatchupsForRound(roundKey) {
  ...
  if (roundKey === "R64") {
    for (const region of REGIONS) {
      ...
      matchups.push({
        region,
        items: ids.map((id) => getR64Matchup(id)).filter(Boolean),
      });
    }
    ...
    for (const region of REGIONS) {
      const items = [];
      for (let i = 0; i < 4; i++)
        items.push(getGeneratedMatchup(laterRoundId(region, 2, i)));
    ...
    for (const region of REGIONS) {
      const items = [];
      for (let i = 0; i < 2; i++)
        items.push(getGeneratedMatchup(laterRoundId(region, 3, i)));
      matchups.push({ region, items: items.filter(Boolean) });
    }
  } else if (roundKey === "E8") {
    for (const region of REGIONS) {
...
function renderMobileRounds() {
  ...
  for (const group of groups) {
    ...
    header.textContent =
      group.region === "Final Four" || group.region === "Championship"
        ? group.region
        : group.region + " Region";
...
function renderApp() {
  ...
  for (const rt of ROUND_TABS) {
    ...
    tab.className =
      "round-tab" + (rt.key === selectedRound ? " round-tab--active" : "");
    ...
    tab.addEventListener("click", () => {
      ...
      tabsBar
        ...
        .forEach((t) => t.classList.remove("round-tab--active"));
...
function updateOddsInPlace(matchupIds) {
  for (const mid of matchupIds) {
    ...
    const recentTick =
      polyData.lastChangeTime && Date.now() - polyData.lastChangeTime < 2000;
    ...
    if (oldBar) {
      ...
      if (pcts.length >= 2) {
        const newText1 =
          Math.round(polyData.team1Prob * 100) +
          "%" +
          (arrow1 ? " " + arrow1 : "");
        const newText2 =
          Math.round(polyData.team2Prob * 100) +
          "%" +
          (arrow2 ? " " + arrow2 : "");
        pcts[0].textContent = newText1;
        pcts[1].textContent = newText2;

        // Update classes while preserving animation classes
        pcts[0].classList.remove(
          "matchup-card__odds-pct--up",
          "matchup-card__odds-pct--down",
        );
        if (d1 > 0.005) {
          pcts[0].classList.add("matchup-card__odds-pct--up");
        } else if (d1 < -0.005) {
          pcts[0].classList.add("matchup-card__odds-pct--down");
        }
        pcts[1].classList.remove(
          "matchup-card__odds-pct--up",
          "matchup-card__odds-pct--down",
        );
        if (d2 > 0.005) {
          pcts[1].classList.add("matchup-card__odds-pct--up");
        } else if (d2 < -0.005) {
          pcts[1].classList.add("matchup-card__odds-pct--down");
        }

        // Tick flash - preserve the arrow color class while animating
        if (recentTick && Math.abs(d1) > 0.001) {
          const wasUp = pcts[0].classList.contains(
            "matchup-card__odds-pct--up",
          );
          const wasDown = pcts[0].classList.contains(
            "matchup-card__odds-pct--down",
          );
        ...
        if (recentTick && Math.abs(d2) > 0.001) {
          const wasUp = pcts[1].classList.contains(
            "matchup-card__odds-pct--up",
          );
          const wasDown = pcts[1].classList.contains(
            "matchup-card__odds-pct--down",
          );
